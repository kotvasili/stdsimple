...
// JS.min v2.1.3
*/
var validation = $.isFunction($.fn.valid) ? 1 : 0;$.fn.isValid = function () {
  return !validation || this.valid();
}, validation && $.validator.setDefaults({ errorClass: "invalid", validClass: "valid", errorPlacement: function errorPlacement(a, b) {
  ...
  $(this).hasClass("step") || (stepper = $(this).closest("ul.stepper"), stepper.find(">li").removeAttr("data-last"), window.innerWidth < 993 || !stepper.hasClass("horizontal") ? $(this).addClass("step").stop().slideDown(400, function () {
    $(this).css({ height: "auto", "margin-bottom": "", display: "inherit" }), a && a(), stepper.find(">li.step").last().attr("data-last", "true");
  }) : $(this).addClass("step").stop().css({ width: "0%", display: "inherit" }).animate({ width: "100%" }, 400, function () {
    $(this).css({ height: "auto", "margin-bottom": "", display: "inherit" }), a && a(), stepper.find(">li.step").last().attr("data-last", "true");
  }));
}, $.fn.deactivateStep = function (a) {
  $(this).hasClass("step") && (stepper = $(this).closest("ul.stepper"), stepper.find(">li").removeAttr("data-last"), window.innerWidth < 993 || !stepper.hasClass("horizontal") ? $(this).stop().css({ transition: "none", "-webkit-transition": "margin-bottom none" }).slideUp(400, function () {
    $(this).removeClass("step").css({ height: "auto", "margin-bottom": "", transition: "margin-bottom .4s", "-webkit-transition": "margin-bottom .4s" }), a && a(), stepper.find(">li").removeAttr("data-last"), stepper.find(">li.step").last().attr("data-last", "true");
  }) : $(this).stop().animate({ width: "0%" }, 400, function () {
    $(this).removeClass("step").hide().css({ height: "auto", "margin-bottom": "", display: "none", width: "" }), a && a(), stepper.find(">li.step").last().attr("data-last", "true");
  }));
}, $.fn.showError = function (a) {
  if (validation) {
    name = this.attr("name"), form = this.closest("form");var b = {};b[name] = a, form.validate().showErrors(b), this.closest("li").addClass("wrong");
  ...
  closable = this.removeClass("active").find(".step-content"), window.innerWidth < 993 || !this.closest("ul").hasClass("horizontal") ? closable.stop().slideUp(300, "easeOutQuad", b) : 1 == a ? closable.animate({ left: "-100%" }, function () {
    closable.css({ display: "none", left: "0%" }, b);
  }) : closable.animate({ left: "100%" }, function () {
    closable.css({ display: "none", left: "0%" }, b);
  });
}, $.fn.openAction = function (a, b) {
  openable = this.removeClass("done").addClass("active").find(".step-content"), window.innerWidth < 993 || !this.closest("ul").hasClass("horizontal") ? openable.slideDown(300, "easeOutQuad", b) : 1 == a ? openable.css({ left: "100%", display: "block" }).animate({ left: "0%" }, b) : openable.css({ left: "-100%", display: "block" }).animate({ left: "0%" }, b);
}, $.fn.activateStepper = function (a) {
  var b = $.extend({ linearStepsNavigation: !0, autoFocusInput: !0, showFeedbackLoader: !0, autoFormCreation: !0 }, a);$(document).on("click", function (a) {
    $(a.target).parents(".stepper").length || $(".stepper.focused").removeClass("focused");
  }), $(this).each(function () {
    var a = $(this);!a.parents("form").length && b.autoFormCreation && (method = a.data("method"), action = a.data("action"), method = method ? method : "GET", action = action ? action : "?", a.wrap('<form action="' + action + '" method="' + method + '"></div>')), a.data("settings", { linearStepsNavigation: b.linearStepsNavigation, autoFocusInput: b.autoFocusInput, showFeedbackLoader: b.showFeedbackLoader }), a.find("li.step.active").openAction(1), a.find(">li").removeAttr("data-last"), a.find(">li.step").last().attr("data-last", "true"), a.on("click", ".step:not(.active)", function () {
      ...
      if (b.preventDefault(), feedback = b ? $(b.target).data("feedback") : void 0, form = a.closest("form"), form.isValid()) {
        if (feedback) return stepper.activateFeedback(), window[feedback].call();form.submit();
      }
    }).on("click", function () {
